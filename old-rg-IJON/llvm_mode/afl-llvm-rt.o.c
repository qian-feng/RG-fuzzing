...
void aif_range(uint32_t addr, int index, int val, int low, int high) { 
  ...
  addr1 = __afl_state;
    ...
    fprintf(fp, "status of id(%llx): %d\n", addr1%MAXMAP_SIZE, __aif_untouched_ptr[addr1%MAXMAP_SIZE]-'0');
#endif
  if (__aif_untouched_ptr[addr1%MAXMAP_SIZE] == TRAPPED)  {
    return;
  }  
  __aif_untouched_ptr[addr1%MAXMAP_SIZE] = TOUCHED; 
...
#ifdef LOGGING
  //FILE *fp = fopen("/data/debug.log", "a+");
  fprintf(fp, "[Before]: Global mindist at %p (%d) is 0x%llx, current dis is 0x%llx, queuesize(%d)\n", addr1%MAXMAP_SIZE, addr1%MAXMAP_SIZE, 0xffffffffffffffff-__afl_max_ptr[addr1%MAXMAP_SIZE], distance, ijon_queue->count);

  // small test of parsing here
  if(cur != NULL) {
    fname = basename(cur->filename);
    memcpy(newcpy, fname, strlen(fname));
    ptr = strtok(newcpy, "_");
    fprintf(fp, "[test]: %s, %s, %s, (int)%d\n", cur->filename, fname, ptr, atoi(ptr));
  }
#endif
// will be deleted from main code
  if(distance == 0) {
    // mute the site
    __aif_untouched_ptr[addr1%MAXMAP_SIZE] = TRAPPED; 
    ...
    while (cur != NULL) {
      temp = cur -> next;
      // here check if this one is associated with this id
      fname = basename(cur->filename);      
      memcpy(newcpy, fname, strlen(fname));
      ptr = strtok(newcpy, "_");
      ptr = strtok(NULL, "_");
#ifdef LOGGING
  fprintf(fp, "[check]: %s, %s, %s\n", cur->filename, fname, ptr);
#endif
      if (atoi(ptr) == addr1%MAXMAP_SIZE) {
...
#ifdef LOGGING
  fprintf(fp, "[aif_range][121]: addr: %p, index(%lu), val(%d), low(%d), high(%d), dist(%lu)\n", addr1%MAXMAP_SIZE, index, val, low, high, distance);
#endif 
  distance = 0xffffffffffffffff-distance; // 
  if(__afl_max_ptr[addr1%MAXMAP_SIZE] < distance) { // store -distance in the memory for comparision. 
    __afl_max_ptr[addr1%MAXMAP_SIZE] = distance; // remember smallest distance
    __aif_max_index_ptr[addr1%MAXMAP_SIZE] = addr1%MAXMAP_SIZE; // remember which watch point this is for
  }
#ifdef LOGGING
  // if(index != -1){
  //   fprintf(fp, "[AIF_RANGE]: index %d, status:%d, distance: %llx\n", index, __aif_untouched_ptr[addr1%MAXMAP_SIZE]-'0', distance);
  // }
  // else {
